...
```
@skills/skills重複処理.md に書いてある手順どおりに、このPCの ~/.cursor/skills/ の重複スキルを整理して。中身が同じものは削除、内容が違うものは skills-maker 版を残す。最後に確認結果を教えて。
```

フォルダを添付できないとき:

```
skills-maker リポジトリの skills/skills重複処理.md に従って、グローバルスキルの重複を整理して。
```
...
- **正本は skills-maker の `skills/` フォルダ**（`install.ps1` がコピーする内容）
